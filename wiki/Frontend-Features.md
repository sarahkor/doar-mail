...
### 📤 Sent
Displays all mails successfully sent to other users.
...
- To edit a draft click the draft item and the compose dialog will get opened.
...
- When a mail is marked as unspam all the urls in it are removed from blacklist.
...
- Users can write a new email using the Compose page:
  ...
  - Fields: To, Subject, Body (in android all fields are required, in awe the 'To' field is required).
...
- Sent mails are added to sender’s Sent folder and receiver’s Inbox
